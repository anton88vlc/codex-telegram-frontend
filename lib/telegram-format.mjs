...
function isLocalFileTarget(value) {
  const target = cleanLocalFileTarget(value);
  return (
    target.startsWith("/") ||
    target.startsWith("~/") ||
    target.startsWith("./") ||
...
function cleanLocalFileTarget(value) {
  return String(value ?? "")
    .trim()
    .replace(/^file:\/\//, "")
    .replace(/^<(.+)>$/, "$1");
...
function renderLinkPlain(label, target) {
  ...
  if (isLocalFileTarget(target)) {
    const fileTarget = cleanLocalFileTarget(target);
    return label && label !== fileTarget ? `${label} — ${fileTarget}` : fileTarget;
  }
  return label || target;
}
      ...
      if (labelEnd > index + 1 && text[labelEnd + 1] === "(") {
        ...
        if (targetEnd > labelEnd + 2) {
          ...
          if (isHttpUrl(target)) {
            ...
            const label = renderInlinePlain(labelRaw);
            const fileTarget = cleanLocalFileTarget(target);
            const displayedTarget = label && label !== fileTarget ? `${label} — ${fileTarget}` : fileTarget;
            out += `<code>${escapeHtml(displayedTarget)}</code>`;
          } else {
...
function collectTable(lines, start) {
  ...
  if (!header || !isTableSeparator(lines[start + 1] ?? "")) {
    return null;
  }
  const rows = [];
  let cursor = start + 2;
  while (cursor < lines.length) {
    ...
    if (!row || row.length < 2 || isTableSeparator(lines[cursor])) {
      break;
    }
    rows.push(row);
    cursor += 1;
...
function renderTablePlain(table) {
  const rows = [table.header, ...table.rows].map((row) => row.map(renderInlinePlain));
  ...
  return rows
    .map((row, index) => {
      const cells = widths.map((width, cellIndex) => padCell(String(row[cellIndex] ?? "").slice(0, width), width));
...
function renderTableHtml(table) {
  return `<pre>${escapeHtml(renderTablePlain(table))}</pre>`;
}
...
function renderTextBlock(block) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (table) {
      flushQuote();
      htmlLines.push(renderTableHtml(table));
      plainLines.push(renderTablePlain(table));
      index = table.end - 1;
      continue;
    }
