...
function formatFastMode({ thread, runtime, config }) {
  const fastMode = normalizeFastMode(
    runtime?.fastMode ??
      runtime?.fast_mode ??
      thread?.fastMode ??
      thread?.fast_mode ??
      config?.statusBarFastMode ??
      config?.fastMode,
  );
...
export function buildStatusBarText({ binding, thread, runtime = null, config = {}, nowMs = Date.now() }) {
  ...
  const parts = [`${model} · ${reasoning} · ${formatFastMode({ thread, runtime, config })}`, `ctx ${formatContext(runtime)}`];
  const rates = [
