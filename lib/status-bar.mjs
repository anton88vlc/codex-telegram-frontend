...
function normalizeFastMode(value) {
  ...
  if (["1", "true", "yes", "on", "fast"].includes(normalized)) {
    return true;
  }
  if (["0", "false", "no", "off", "slow"].includes(normalized)) {
    return false;
  }
  return null;
}
...
function hasOwn(object, key) {
  return object != null && Object.prototype.hasOwnProperty.call(Object(object), key);
}

function formatFastMode({ thread, runtime, config, codexConfig }) {
  const override = normalizeFastMode(config?.statusBarFastMode);
  if (override !== null) {
    return `fast ${override ? "on" : "off"}`;
  }

  const candidates = [
    { present: hasOwn(runtime, "fastMode"), value: runtime?.fastMode },
    { present: hasOwn(runtime, "fast_mode"), value: runtime?.fast_mode },
    // In Codex config, null service_tier means the standard non-fast tier.
    { present: hasOwn(codexConfig, "service_tier"), value: codexConfig?.service_tier, nullMeansOff: true },
    { present: hasOwn(codexConfig, "serviceTier"), value: codexConfig?.serviceTier, nullMeansOff: true },
    { present: hasOwn(codexConfig, "fastMode"), value: codexConfig?.fastMode },
    { present: hasOwn(codexConfig, "fast_mode"), value: codexConfig?.fast_mode },
    { present: hasOwn(thread, "fastMode"), value: thread?.fastMode },
    { present: hasOwn(thread, "fast_mode"), value: thread?.fast_mode },
    { present: hasOwn(config, "fastMode"), value: config?.fastMode },
  ];
  for (const candidate of candidates) {
    if (!candidate.present) {
      continue;
    }
    const fastMode = candidate.nullMeansOff && candidate.value == null ? false : normalizeFastMode(candidate.value);
    if (fastMode !== null) {
      return `fast ${fastMode ? "on" : "off"}`;
    }
  }
  return "fast n/a";
}
...
function formatRateLimit(limit, { label, nowMs = Date.now() } = {}) {
  ...
  return `${windowLabel} ${formatPercent(remaining)} reset ${reset} (${until})`;
}
...
function formatCompactStatus(binding, config) {
  ...
  const queueLength = Array.isArray(binding?.turnQueue) ? binding.turnQueue.filter((item) => item?.prompt).length : 0;
  const status = [];
  if (!binding?.statusBarMessageId) {
    status.push("unpinned");
  }
  if (activity === "running") {
    status.push(activityLabel);
  }
  if (queueLength) {
    status.push(`queue ${queueLength}${binding?.queuePaused ? " paused" : ""}`);
  }
  if (config.outboundSyncEnabled === false) {
    status.push("mirror off");
  }
...
function pushDateTimeEntity(entities, { text, needle, unixTime, searchFrom = 0, dateTimeFormat = "t" }) {
  if (!needle || !Number.isFinite(Number(unixTime)) || Number(unixTime) <= 0) {
    return searchFrom;
  }
  const offset = text.indexOf(needle, searchFrom);
  if (offset < 0) {
    return searchFrom;
  }
  entities.push({
    ...
    unix_time: Math.floor(Number(unixTime)),
...
export function extractRuntimeStatusFromLine(line) {
  ...
  return {
    ...
    fastMode: payload.info?.fast_mode ?? payload.info?.fastMode ?? payload.fast_mode ?? payload.fastMode ?? null,
  };
}
...
export function buildStatusBarText({ binding, thread, runtime = null, config = {}, codexConfig = null, nowMs = Date.now() }) {
  const model = normalizeText(codexConfig?.model) || normalizeText(thread?.model) || normalizeText(thread?.model_provider) || "n/a";
  const reasoning = normalizeText(codexConfig?.model_reasoning_effort) || normalizeText(thread?.reasoning_effort) || "n/a";
  const runtimeRateLimits = runtime?.rateLimits ?? {};

  const parts = [
    `${model} · ${reasoning} · ${formatFastMode({ thread, runtime, config, codexConfig })}`,
    `ctx ${formatContext(runtime)}`,
  ];
  const rates = [
  ...
  parts.push(formatCompactStatus(binding, config));

  return parts.join(" · ");
}

export function buildStatusBarEntities({ text, runtime = null }) {
  const entities = [];
  let searchFrom = 0;
  for (const limit of [runtime?.rateLimits?.primary, runtime?.rateLimits?.secondary]) {
    const reset = formatLocalTimeFromSeconds(limit?.resets_at);
    if (reset === "n/a") {
      continue;
    }
    searchFrom = pushDateTimeEntity(entities, {
      text,
      needle: reset,
      unixTime: limit.resets_at,
      searchFrom,
      dateTimeFormat: "t",
    });
  }
...
export function buildStatusBarMessage(params) {
  const text = buildStatusBarText(params);
  return {
    text,
    entities: buildStatusBarEntities({ text, runtime: params.runtime }),
  };
}
