...
export function buildStatusBarText({ binding, thread, runtime = null, config = {}, codexConfig = null, nowMs = Date.now() }) {
  const model = normalizeText(thread?.model) || normalizeText(thread?.model_provider) || normalizeText(codexConfig?.model) || "n/a";
  const reasoning = normalizeText(thread?.reasoning_effort) || normalizeText(codexConfig?.model_reasoning_effort) || "n/a";
