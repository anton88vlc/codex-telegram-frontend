...
function formatCompactStatus(binding, config) {
  ...
  const queueLabel = queueLength ? `queue ${queueLength}${binding?.queuePaused ? " paused" : ""}` : null;
  const mirror = config.outboundSyncEnabled === false ? "mirror off" : "mirror on";
  return [pinned, activityLabel, queueLabel, mirror].filter(Boolean).join(", ");
}
...
export function buildStatusBarText({ binding, thread, runtime = null, config = {}, nowMs = Date.now() }) {
  ...
  const lines = [`${model} | ${reasoning} | ${formatFastMode({ thread, runtime, config })}`, `ctx: ${formatContext(runtime)}`];
  const rates = [
