...
export const DEFAULT_BOT_COMMANDS = [
  ...
  { command: "cancel", description: "Interrupt the current Codex turn" },
  { command: "interrupt", description: "Alias for /cancel" },
];
...
export function buildBotInstallPolishPlan({
  ...
  return {
    ...
    avatarPath: includeAvatar ? avatarPath : null,
  };
}
...
export function buildBotInstallPolishOperations(plan) {
  ...
  if (plan.avatarPath) {
    operations.push({
      ...
      args: { photoPath: plan.avatarPath },
    });
  }
  return operations;
}
...
export function formatBotInstallPolishPlan(plan) {
  ...
  for (const operation of operations) {
    ...
    } else if (operation.name === "setMyProfilePhoto") {
      lines.push(`- profile photo: ${operation.args.photoPath}`);
    }
  }
