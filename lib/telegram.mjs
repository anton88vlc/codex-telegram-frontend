...
export async function callTelegramMultipartApi(token, method, fields = {}, files = {}) {
...
function encodeTelegramFilePath(filePath) {
...
export async function downloadTelegramFile(token, { filePath }) {
  const encodedPath = encodeTelegramFilePath(filePath);
...
export async function sendMessage(
  token,
  { chatId, messageThreadId = null, text, replyToMessageId = null, parseMode = null, entities = null, replyMarkup = null },
) {
  ...
  return callTelegramApi(token, "sendMessage", {
    ...
    message_thread_id: messageThreadId ?? undefined,
    text,
    parse_mode: hasEntities ? undefined : parseMode || undefined,
...
export async function sendPhoto(
  ...
  if (!photoPath) {
    throw new Error("missing photoPath for Telegram sendPhoto");
  }
  const hasEntities = Array.isArray(captionEntities) && captionEntities.length > 0;
  return callTelegramMultipartApi(
    token,
    "sendPhoto",
    {
      chat_id: chatId,
      ...
      parse_mode: hasEntities ? undefined : parseMode || undefined,
      caption_entities: hasEntities ? captionEntities : undefined,
      reply_to_message_id: replyToMessageId ?? undefined,
      reply_parameters: replyToMessageId ? { message_id: replyToMessageId } : undefined,
      allow_sending_without_reply: true,
    },
...
export async function editMessageText(
  token,
  { chatId, messageId, text, parseMode = null, entities = null, replyMarkup = null },
) {
  const hasEntities = Array.isArray(entities) && entities.length > 0;
  try {
...
export async function deleteForumTopic(token, { chatId, messageThreadId }) {
  return callTelegramApi(token, "deleteForumTopic", {
    chat_id: chatId,
    message_thread_id: messageThreadId,
  });
}
...
export async function setMyProfilePhoto(token, { photoPath }) {
  if (!photoPath) {
    throw new Error("missing photoPath for Telegram setMyProfilePhoto");
  }
  return callTelegramMultipartApi(
    token,
    "setMyProfilePhoto",
