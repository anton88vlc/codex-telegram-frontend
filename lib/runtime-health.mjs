...
async function inspectAppServer(config, timeoutMs = 3_000) {
  const label = appServerTransportLabel(config);
  if (!hasAppServerTransport(config)) {
    return {
      ok: false,
      transport: label,
      error: "disabled",
  ...
  const client = new AppServerProtocolClient({
    ...appServerClientOptions(config, {
      connectTimeoutMs: timeoutMs,
      requestTimeoutMs: timeoutMs,
    }),
    clientInfo: {
      name: "codex-telegram-frontend-health",
      title: "Codex Telegram Frontend Health",
      version: "0.1.0",
    },
  });
  try {
    const initializeResult = await client.connect();
    await client.request("thread/list", { limit: 1 }, { timeoutMs });
    return {
      ok: true,
      transport: label,
...
export async function buildSelfCheckReport({ config, state }) {
  ...
  if (bindingThreadIds.length > 0) {
    try {
      threadRows = await getThreadsByIdsFromStore(config, bindingThreadIds);
    } catch (error) {
  ...
  if (!threadsDbExists) {
    if (appServer.ok && config.appServerThreadStoreEnabled !== false) {
      notes.push(`threads DB missing, using app-server ThreadStore: ${config.threadsDbPath}`);
    } else {
      warnings.push(`threads DB missing: ${config.threadsDbPath}`);
    }
  }
...
export function formatSelfCheckReport(report, config) {
  const lines = [
    ...
    `legacy app-server helper: ${config.nativeFallbackHelperPath} (${report.fallbackHelperExists ? "ok" : "missing"})`,
    ...
    `app-server: ${report.appServer.transport || appServerTransportLabel(config)} (${report.appServer.ok ? "reachable" : report.appServer.error})`,
