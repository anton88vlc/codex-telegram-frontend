...
const DEFAULT_SUBSCRIBE_FAILURE_MAX_DELAY_MS = 10 * 60 * 1000;
...
export class AppServerLiveStream {
  constructor({
    ...
    subscribeFailureMaxDelayMs = DEFAULT_SUBSCRIBE_FAILURE_MAX_DELAY_MS,
    maxQueuedEvents = 500,
    ...
    this.subscribeFailureMaxDelayMs = subscribeFailureMaxDelayMs;
  ...
  async subscribe(threadId) {
    ...
    try {
      ...
      const retryDelayMs = Math.min(
        this.reconnectMs * 2 ** Math.max(0, failures - 1),
        Math.max(this.reconnectMs, this.subscribeFailureMaxDelayMs),
      );
