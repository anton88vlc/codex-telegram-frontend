...
export function renderNativeSendError(error) {
  ...
  const appServerError = attemptError(error, "app-server") || attemptError(error, "app-server-fallback");
  ...
  if (kind === "fallback_failed") {
    return [
      ...
      "app-control failed, and app-server failed too. Open Codex.app, then retry. Use `--remote-debugging-port=9222` only if you are deliberately testing the optional app-control lane.",
  ...
  if (kind === "app_control_unavailable" || /fetch failed|econnrefused|no page targets found|couldn't connect/i.test(raw)) {
    return [
      ...
      "The bridge is alive. Normal Telegram sends should use app-server; launch Codex with `--remote-debugging-port=9222` only if this topic is intentionally pinned to app-control.",
    ].join("\n");
  }
...
export function appendTransportNotice(replyText, result = {}) {
  ...
  return [
    text,
    "",
    "Transport note: delivered through app-server after app-control failed. That is fine for the normal Telegram path; use `--remote-debugging-port=9222` only when you intentionally want app-control behavior.",
  ].join("\n");
}
