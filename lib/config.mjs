...
import {
  DEFAULT_BINDING_HOT_MAX_AGE_MS,
  DEFAULT_BINDING_WARM_MAX_AGE_MS,
...
const DEFAULT_OUTBOUND_MIRROR_MAX_BINDINGS_PER_POLL = 25;
const DEFAULT_OUTBOUND_MIRROR_PHASES = ["commentary", "final_answer", "image_generation"];
const DEFAULT_APP_SERVER_STREAM_CONNECT_TIMEOUT_MS = 1_200;
const DEFAULT_APP_SERVER_STREAM_REQUEST_TIMEOUT_MS = 10_000;
...
const DEFAULT_APP_SERVER_STREAM_SUBSCRIBE_MAX_ATTEMPTS_PER_POLL = 3;
const DEFAULT_APP_SERVER_STREAM_SUBSCRIBE_FAILURE_MAX_DELAY_MS = 10 * 60 * 1000;
const DEFAULT_APP_SERVER_STREAM_LOG_SUBSCRIBE_ERRORS = false;
const DEFAULT_DRAFT_STREAMING_MAX_CHARS = 1200;
const DEFAULT_DRAFT_STREAMING_ERROR_COOLDOWN_MS = 10 * 60 * 1000;
const DEFAULT_TYPING_HEARTBEAT_ERROR_COOLDOWN_MS = 2 * 60 * 1000;
...
function normalizeOutboundMirrorPhases(value) {
  const allowed = new Set(["commentary", "final_answer", "image_generation"]);
...
export async function loadConfig(configPath) {
  ...
  return {
    ...
    appServerTransport: normalizeText(fromFile?.appServerTransport) || DEFAULT_APP_SERVER_TRANSPORT,
    appServerCommand: normalizeText(fromFile?.appServerCommand),
    appServerArgs: Array.isArray(fromFile?.appServerArgs) ? fromFile.appServerArgs.map(String).filter(Boolean) : [],
    appServerCwd: normalizeText(fromFile?.appServerCwd),
    appServerEnv: fromFile?.appServerEnv && typeof fromFile.appServerEnv === "object" ? fromFile.appServerEnv : {},
    appServerSocketPath: normalizeText(fromFile?.appServerSocketPath),
    ...
    appServerThreadStoreEnabled: fromFile?.appServerThreadStoreEnabled !== false,
    ...
    appServerStreamRequestTimeoutMs: Number.isFinite(fromFile?.appServerStreamRequestTimeoutMs)
      ? Math.max(500, Number(fromFile.appServerStreamRequestTimeoutMs))
      : DEFAULT_APP_SERVER_STREAM_REQUEST_TIMEOUT_MS,
    ...
    appServerStreamSubscribeMaxAttemptsPerPoll: Number.isFinite(fromFile?.appServerStreamSubscribeMaxAttemptsPerPoll)
      ? Math.max(1, Math.min(20, Number(fromFile.appServerStreamSubscribeMaxAttemptsPerPoll)))
      : DEFAULT_APP_SERVER_STREAM_SUBSCRIBE_MAX_ATTEMPTS_PER_POLL,
    appServerStreamSubscribeFailureMaxDelayMs: Number.isFinite(fromFile?.appServerStreamSubscribeFailureMaxDelayMs)
      ? Math.max(0, Number(fromFile.appServerStreamSubscribeFailureMaxDelayMs))
      : DEFAULT_APP_SERVER_STREAM_SUBSCRIBE_FAILURE_MAX_DELAY_MS,
    appServerStreamSubscribeHotOnly: fromFile?.appServerStreamSubscribeHotOnly !== false,
    appServerStreamLogSubscribeErrors:
      typeof fromFile?.appServerStreamLogSubscribeErrors === "boolean"
        ? fromFile.appServerStreamLogSubscribeErrors
        : DEFAULT_APP_SERVER_STREAM_LOG_SUBSCRIBE_ERRORS,
    draftStreamingEnabled: fromFile?.draftStreamingEnabled === true,
    ...
    outboundMirrorMaxBindingsPerPoll: Number.isFinite(fromFile?.outboundMirrorMaxBindingsPerPoll)
      ? Math.max(0, Math.min(500, Number(fromFile.outboundMirrorMaxBindingsPerPoll)))
      : DEFAULT_OUTBOUND_MIRROR_MAX_BINDINGS_PER_POLL,
    ...
    bindingHotMaxAgeMs: Number.isFinite(fromFile?.bindingHotMaxAgeMs)
      ? Math.max(0, Number(fromFile.bindingHotMaxAgeMs))
      : DEFAULT_BINDING_HOT_MAX_AGE_MS,
    bindingWarmMaxAgeMs: Number.isFinite(fromFile?.bindingWarmMaxAgeMs)
      ? Math.max(0, Number(fromFile.bindingWarmMaxAgeMs))
      : DEFAULT_BINDING_WARM_MAX_AGE_MS,
    typingHeartbeatErrorCooldownMs: Number.isFinite(fromFile?.typingHeartbeatErrorCooldownMs)
      ? Math.max(0, Number(fromFile.typingHeartbeatErrorCooldownMs))
      : DEFAULT_TYPING_HEARTBEAT_ERROR_COOLDOWN_MS,
    ...
    eventLogMaxBytes:
      fromFile?.eventLogMaxBytes === 0 || fromFile?.eventLogMaxBytes === false
        ? 0
        : Number.isFinite(fromFile?.eventLogMaxBytes)
          ? Math.max(64 * 1024, Number(fromFile.eventLogMaxBytes))
