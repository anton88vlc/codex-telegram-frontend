...
export function formatVoiceTranscriptBubble(transcripts = []) {
  ...
  items.forEach((item, index) => {
    ...
    const meta = [item.kind, duration, item.detectedLanguage ? `lang ${item.detectedLanguage}` : null]
      .filter(Boolean)
      .join(", ");
    if (items.length > 1) {
      lines.push("", `${index + 1}. ${meta || "audio"}`);
    } else if (meta) {
      lines.push(meta);
    }
...
export function formatVoiceTranscriptionReceipt(transcripts = []) {
  const count = transcripts.length || 1;
  return count === 1 ? "Voice transcribed. Sending to Codex..." : `${count} voice messages transcribed. Sending to Codex...`;
}
