...
export function formatUnboundGroupFallbackBubble({ message, promptText, attachmentRefs = [], voiceRefs = [] }) {
  ...
  const lines = [`**${sender} from ${source}**`];
  const preview = truncatePreview(promptText);
  if (preview) {
    lines.push("", quoteMarkdownBlock(preview));
  }
  if (attachmentRefs.length) {
    lines.push("", `_${attachmentRefs.length} attachment${attachmentRefs.length === 1 ? "" : "s"} attached._`);
  }
  if (voiceRefs.length) {
    lines.push("", `_${voiceRefs.length === 1 ? "voice note" : `${voiceRefs.length} voice notes`} attached._`);
  }
