...
import { listProjectThreadsFromStore } from "./thread-store.mjs";
...
export function renderHelp(config) {
  ...
  return [
    ...
    "`/steer <text>` - add guidance to the currently running Codex turn",
    "`/cancel` or `/interrupt` - stop the currently running Codex turn",
...
export async function handleCommand({
  ...
  listProjectThreadsFn = (_threadsDbPath, projectRoot, options) => listProjectThreadsFromStore(config, projectRoot, options),
  ...
  steerCodexTurnFn = steerCodexTurn,
  interruptCodexTurnFn = interruptCodexTurn,
  ...
  switch (parsed.command) {
    ...
    case "/interrupt": {
      ...
      if (!binding.currentTurn) {
        await replyFn(config.botToken, message, "No active Codex turn here. Nothing to interrupt.");
        return true;
      }
      const turnId = getCurrentCodexTurnId(binding);
      if (!turnId) {
        await replyFn(
          config.botToken,
          message,
          "I can see an active turn locally, but Codex has not exposed its app-server turn id yet. Try again in a few seconds.",
        );
        return true;
      }
      try {
        await interruptCodexTurnFn(config, {
          threadId: binding.threadId,
          turnId,
        });
        binding.currentTurn = null;
        binding.lastTransportPath = "app-server-turn-interrupt";
        binding.updatedAt = new Date().toISOString();
        delete binding.lastTransportErrorAt;
        delete binding.lastTransportErrorKind;
        await replyFn(config.botToken, message, "Interrupted the current Codex turn.");
      } catch (error) {
        binding.updatedAt = new Date().toISOString();
        await replyFn(config.botToken, message, renderCodexControlErrorFn(error));
      }
    ...
    case "/steer": {
      ...
      try {
        ...
        binding.lastTransportPath = "app-server-turn-steer";
        binding.updatedAt = new Date().toISOString();
        delete binding.lastTransportErrorAt;
        delete binding.lastTransportErrorKind;
