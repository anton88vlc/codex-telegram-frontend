...
function threadUpdatedAtMs(thread) {
  const explicit = Number(thread?.updated_at_ms);
...
export function isThreadFreshEnoughForAutoSync(thread, { maxThreadAgeMs = 0, nowMs = Date.now() } = {}) {
  const maxAge = Number(maxThreadAgeMs);
  if (!Number.isFinite(maxAge) || maxAge <= 0) {
    return true;
  }
  const updatedAt = threadUpdatedAtMs(thread);
  return updatedAt > 0 && Number(nowMs) - updatedAt <= maxAge;
}
...
export function buildProjectSyncPlan({
  ...
  const desiredThreads = [...threads]
    .filter((thread) => isThreadFreshEnoughForAutoSync(thread, { maxThreadAgeMs, nowMs }))
