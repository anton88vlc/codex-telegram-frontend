...
import { getThreadByIdFromStore } from "./thread-store.mjs";

export async function renderBindingStatus(
  config,
  bindingKey,
  binding,
  { getThreadByIdFn = (_threadsDbPath, threadId) => getThreadByIdFromStore(config, threadId) } = {},
) {
  const lines = [
    "**Current binding**",
...
export async function renderHealth(
  ...
  const lines = [
    ...
    `app server: \`${appServerTransportLabel(config)}\``,
    ...
    `delivery: app-server ${eventSummary.appServerSends || 0}, app-control ${eventSummary.appControlSends}, app-server fallback ${eventSummary.appServerFallbackSends}, native errors ${eventSummary.nativeSendErrors}, ops dm fallbacks ${eventSummary.opsDmFallbacks}`,
    stateDoctor
