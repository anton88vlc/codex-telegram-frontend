...
function shouldSkipRolloutProgressBecauseAppServerIsLive({ config, binding, message }) {
  if (config?.appServerStreamEnabled === false) {
    return false;
  }
  if (!binding?.currentTurn || binding.currentTurn.progressSource !== "app-server") {
    return false;
  }
  if (!Array.isArray(binding.currentTurn.progressItems) || binding.currentTurn.progressItems.length === 0) {
    return false;
  }
  return isCommentaryAssistantMirrorMessage(message) || isPlanMirrorMessage(message);
}
...
export async function syncOutboundMirrors({
  ...
  const bindingEntries = Object.entries(state.bindings ?? {}).filter(([bindingKey, binding]) => {
    ...
    return isOutboundMirrorBindingEligible(binding);
  });
  ...
  let suppressed = 0;
  let changed = false;
  let stopAfterCurrentBinding = false;
  ...
  for (const [bindingKey, binding] of selectedBindingEntries) {
    ...
    for (let index = 0; index < queuedMessages.length; index += 1) {
      ...
      if (shouldSkipRolloutProgressBecauseAppServerIsLive({ config, binding, message })) {
        lastSignature = message.signature;
        binding.lastRolloutProgressSkippedAt = message.timestamp || new Date().toISOString();
        suppressed += 1;
        changed = true;
        continue;
      }
      ...
      try {
        ...
        stopAfterCurrentBinding =
          onTelegramRateLimitFn(error, {
            stage: "outbound_mirror",
            bindingKey,
            threadId: binding.threadId,
          }) || stopAfterCurrentBinding;
        break;
