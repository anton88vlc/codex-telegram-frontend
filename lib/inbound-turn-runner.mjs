...
import { makeAppServerStreamClientFactory } from "./app-server-borrowed-client.mjs";
import { handlePendingUserInputReply } from "./app-server-approvals.mjs";
...
export async function handlePlainText({
  ...
  handlePendingUserInputReplyFn = handlePendingUserInputReply,
}) {
  ...
  if (binding) {
    const handledPendingInput = await handlePendingUserInputReplyFn({
      config,
      state,
      message,
      responseText: prompt,
  ...
  try {
    ...
    const result = await sendNativeTurnFn({
      ...
      appServerOptions: appServerClientOptions(config),
      appServerClientFactory: makeAppServerStreamClientFactory(appServerStream),
    ...
    if (config.nativeWaitForReply === false) {
      ...
      binding.currentTurn = {
        ...
        codexProgressMessageId: Number.isInteger(receiptMessageId) ? receiptMessageId : undefined,
        appServerTurnId: result?.turn?.id || binding.currentTurn?.appServerTurnId || undefined,
        sendOnly: true,
