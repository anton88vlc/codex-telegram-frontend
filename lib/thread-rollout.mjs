import { createHash } from "node:crypto";
import fs from "node:fs/promises";
import os from "node:os";
import path from "node:path";

import { normalizeText } from "./message-routing.mjs";

export const DEFAULT_OUTBOUND_PHASES = ["final_answer"];
export const IMAGE_GENERATION_PHASE = "image_generation";
const MAX_FULL_RESYNC_BYTES = 5 * 1024 * 1024;
const MAX_TAIL_RESYNC_BYTES = 128 * 1024 * 1024;
const MAX_PLAN_ITEMS = 6;
const PLAN_STEP_LIMIT = 140;
const USER_FILES_HEADER = "# Files mentioned by the user:";
const CODEX_APP_DIRECTIVE_LINE = /^::[a-z][\w-]*\{.*\}\s*$/i;
const ENVIRONMENT_CONTEXT_BLOCK_PATTERN = /<environment_context>[\s\S]*?(?:<\/environment_context>|$)/gi;
const MEMORY_CITATION_BLOCK_PATTERN = /<oai-mem-citation>[\s\S]*?(?:<\/oai-mem-citation>|$)/gi;
const MEMORY_CITATION_CHILD_BLOCK_PATTERN = /<(?:citation_entries|rollout_ids)>[\s\S]*?<\/(?:citation_entries|rollout_ids)>/gi;
const HEARTBEAT_BLOCK_PATTERN = /<heartbeat\b[^>]*>[\s\S]*?(?:<\/heartbeat>|$)/gi;
const HEARTBEAT_MESSAGE_PATTERN = /<message\b[^>]*>([\s\S]*?)(?:<\/message>|$)/i;
const GENERATED_IMAGES_ROOT = path.join(os.homedir(), ".codex", "generated_images");

function normalizePhases(phases) {
  const normalized = Array.from(new Set((Array.isArray(phases) ? phases : DEFAULT_OUTBOUND_PHASES).map(String)));
  return normalized.length ? normalized : [...DEFAULT_OUTBOUND_PHASES];
}

function extractTextParts(content) {
  if (!Array.isArray(content)) {
    return "";
  }
  return content
    .filter((item) => item?.type === "output_text" || item?.type === "input_text")
    .map((item) => String(item?.text ?? ""))
    .join("");
}

export function cleanupMirrorUserText(text) {
  const normalized = normalizeText(text).replace(ENVIRONMENT_CONTEXT_BLOCK_PATTERN, "").replace(/\n{3,}/g, "\n\n").trim();
  if (!normalized) {
    return null;
  }
  if (normalized.startsWith("# AGENTS.md instructions")) {
    return null;
  }
  if (normalized.startsWith("<turn_aborted>")) {
    return null;
  }
  if (normalized.startsWith("<heartbeat>")) {
    return null;
  }
  if (normalized.startsWith(USER_FILES_HEADER)) {
    const requestMarker = "## My request for Codex:";
    const files = [];
    for (const line of normalized.split("\n")) {
      const stripped = line.trim();
      if (stripped.startsWith("## ") && stripped.endsWith(":") && stripped !== requestMarker) {
        files.push(stripped.slice(3, -1).trim());
      }
    }

    const request = normalized.includes(requestMarker) ? normalized.split(requestMarker, 2)[1].trim() : normalized;
    const cleanedLines = [];
    let imageCount = 0;
    for (const line of request.split("\n")) {
      const stripped = line.trim();
      if (stripped.startsWith("<image ") || stripped === "</image>") {
        if (stripped.startsWith("<image ")) {
          imageCount += 1;
        }
        continue;
      }
      cleanedLines.push(line);
    }

    const body = cleanedLines.join("\n").trim();
    const parts = [];
    if (files.length) {
      parts.push(`[files]\n${files.map((name) => `- ${name}`).join("\n")}`);
    }
    if (imageCount) {
      parts.push(`_${imageCount} image${imageCount === 1 ? "" : "s"} attached._`);
    }
    if (body) {
      parts.push(body);
    }
    return parts.join("\n\n").trim() || null;
  }
  return normalized;
}

function decodeXmlText(value) {
  return normalizeText(value)
    .replaceAll("&lt;", "<")
    .replaceAll("&gt;", ">")
    .replaceAll("&quot;", '"')
    .replaceAll("&apos;", "'")
    .replaceAll("&amp;", "&")
    .trim();
}

function extractHeartbeatMessages(text) {
  const messages = [];
  for (const match of String(text ?? "").matchAll(HEARTBEAT_BLOCK_PATTERN)) {
    const block = match[0] || "";
    const messageMatch = block.match(HEARTBEAT_MESSAGE_PATTERN);
    const message = decodeXmlText(messageMatch?.[1] || "");
    if (message) {
      messages.push(message);
    }
  }
  return messages;
}

export function cleanupMirrorAssistantText(text) {
  const normalized = normalizeText(text).replace(/\r\n/g, "\n");
  if (!normalized) {
    return null;
  }
  const heartbeatMessages = extractHeartbeatMessages(normalized);
  const withoutInternalBlocks = normalized
    .replace(MEMORY_CITATION_BLOCK_PATTERN, "")
    .replace(MEMORY_CITATION_CHILD_BLOCK_PATTERN, "")
    .replace(HEARTBEAT_BLOCK_PATTERN, "");
  const cleaned = withoutInternalBlocks
    .split("\n")
    .filter((line) => !CODEX_APP_DIRECTIVE_LINE.test(line.trim()))
    .join("\n")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
  if (cleaned) {
    return cleaned;
  }
  return heartbeatMessages.join("\n\n").trim() || null;
}

export function getGeneratedImagePath({ threadId, imageId }) {
  const normalizedThreadId = normalizeText(threadId);
  const normalizedImageId = normalizeText(imageId);
  if (!normalizedThreadId || !normalizedImageId) {
    return null;
  }
  return path.join(GENERATED_IMAGES_ROOT, normalizedThreadId, `${normalizedImageId}.png`);
}

function normalizeMediaSignature(media) {
  if (!Array.isArray(media) || media.length === 0) {
    return "";
  }
  return media
    .map((item) => [normalizeText(item?.type), normalizeText(item?.source), normalizeText(item?.imageId || item?.path)].join(":"))
    .join("\n");
}

export function makeOutboundMirrorSignature({ role = "assistant", phase, text, media = [] }) {
  const normalizedRole = normalizeText(role) || "assistant";
  const normalizedPhase = normalizeText(phase) || "final_answer";
  const normalizedText = normalizeText(text).replace(/\r\n/g, "\n");
  const normalizedMedia = normalizeMediaSignature(media);
  if (normalizedRole === "assistant") {
    const payload = normalizedMedia ? `${normalizedPhase}\n${normalizedText}\n${normalizedMedia}` : `${normalizedPhase}\n${normalizedText}`;
    return createHash("sha1").update(payload).digest("hex");
  }
  const payload = normalizedMedia ? `${normalizedRole}\n${normalizedText}\n${normalizedMedia}` : `${normalizedRole}\n${normalizedText}`;
  return createHash("sha1").update(payload).digest("hex");
}

function compactPlanStep(text) {
  const normalized = normalizeText(text).replace(/\s+/g, " ");
  if (normalized.length <= PLAN_STEP_LIMIT) {
    return normalized;
  }
  return `${normalized.slice(0, PLAN_STEP_LIMIT - 3).trimEnd()}...`;
}

function parseUpdatePlanArguments(value) {
  if (value && typeof value === "object") {
    return value;
  }
  const raw = normalizeText(value);
  if (!raw) {
    return null;
  }
  try {
    const parsed = JSON.parse(raw);
    return parsed && typeof parsed === "object" ? parsed : null;
  } catch {
    return null;
  }
}

function formatPlanStatusMarker(status) {
  switch (normalizeText(status)) {
    case "completed":
      return "[x]";
    case "in_progress":
      return "[>]";
    case "pending":
      return "[ ]";
    default:
      return "[-]";
  }
}

export function formatUpdatePlanMirrorText(argumentsValue) {
  const parsed = parseUpdatePlanArguments(argumentsValue);
  const items = Array.isArray(parsed?.plan)
    ? parsed.plan
        .map((item) => ({
          step: compactPlanStep(item?.step),
          status: normalizeText(item?.status) || "pending",
        }))
        .filter((item) => item.step)
    : [];
  if (!items.length) {
    return null;
  }

  const completed = items.filter((item) => item.status === "completed").length;
  const visible = items.slice(0, MAX_PLAN_ITEMS);
  const lines = ["**Todo**", `${completed}/${items.length} done`];
  for (let index = 0; index < visible.length; index += 1) {
    const item = visible[index];
    lines.push(`${index + 1}. ${formatPlanStatusMarker(item.status)} ${item.step}`);
  }
  if (items.length > visible.length) {
    lines.push(`... +${items.length - visible.length} more`);
  }
  return lines.join("\n");
}

export function parseThreadMirrorLine(line, { phases = DEFAULT_OUTBOUND_PHASES, threadId = null } = {}) {
  const normalizedLine = String(line ?? "").trim();
  if (!normalizedLine) {
    return null;
  }

  let parsed;
  try {
    parsed = JSON.parse(normalizedLine);
  } catch {
    return null;
  }

  if (parsed?.type !== "response_item") {
    return null;
  }
  const payload = parsed?.payload ?? {};
  const normalizedPhases = normalizePhases(phases);

  if (payload?.type === "function_call" && normalizeText(payload?.name) === "update_plan") {
    if (!normalizedPhases.includes("commentary")) {
      return null;
    }
    const text = formatUpdatePlanMirrorText(payload?.arguments);
    if (!text) {
      return null;
    }
    return {
      timestamp: parsed?.timestamp ?? null,
      role: "plan",
      phase: "update_plan",
      text,
      signature: makeOutboundMirrorSignature({ role: "plan", phase: "update_plan", text }),
    };
  }

  if (payload?.type === "image_generation_call") {
    if (!normalizedPhases.includes(IMAGE_GENERATION_PHASE)) {
      return null;
    }
    const imageId = normalizeText(payload?.id);
    const hasImageResult = Boolean(normalizeText(payload?.result));
    const imagePath = getGeneratedImagePath({ threadId, imageId });
    if (!imageId || !hasImageResult || !imagePath) {
      return null;
    }
    const phase = IMAGE_GENERATION_PHASE;
    const text = "Generated image";
    const media = [
      {
        type: "photo",
        source: "codex_image_generation",
        imageId,
        path: imagePath,
        mimeType: "image/png",
      },
    ];
    return {
      timestamp: parsed?.timestamp ?? null,
      role: "assistant",
      phase,
      text,
      media,
      signature: makeOutboundMirrorSignature({ role: "assistant", phase, text, media }),
    };
  }

  const role = normalizeText(payload?.role);
  if (payload?.type !== "message" || !["assistant", "user"].includes(role)) {
    return null;
  }

  if (role === "assistant") {
    const phase = normalizeText(payload?.phase);
    if (!normalizedPhases.includes(phase)) {
      return null;
    }
    const text = cleanupMirrorAssistantText(extractTextParts(payload?.content));
    if (!text) {
      return null;
    }
    return {
      timestamp: parsed?.timestamp ?? null,
      role,
      phase,
      text,
      signature: makeOutboundMirrorSignature({ role, phase, text }),
    };
  }

  const text = cleanupMirrorUserText(extractTextParts(payload?.content));
  if (!text) {
    return null;
  }
  return {
    timestamp: parsed?.timestamp ?? null,
    role,
    phase: null,
    text,
    signature: makeOutboundMirrorSignature({ role, text }),
  };
}

export function parseThreadMirrorChunk(text, { carry = "", phases = DEFAULT_OUTBOUND_PHASES, threadId = null } = {}) {
  const combined = `${String(carry ?? "")}${String(text ?? "")}`;
  if (!combined) {
    return {
      messages: [],
      trailingPartial: "",
    };
  }

  const rawLines = combined.replace(/\r\n/g, "\n").split("\n");
  const trailingPartial = combined.endsWith("\n") ? "" : rawLines.pop() ?? "";
  const messages = [];

  for (const rawLine of rawLines) {
    const parsed = parseThreadMirrorLine(rawLine, { phases, threadId });
    if (parsed) {
      messages.push(parsed);
    }
  }

  return {
    messages,
    trailingPartial,
  };
}

export function parseAssistantMirrorLine(line, { phases = DEFAULT_OUTBOUND_PHASES, threadId = null } = {}) {
  const parsed = parseThreadMirrorLine(line, { phases, threadId });
  return parsed?.role === "assistant" ? parsed : null;
}

export function parseAssistantMirrorChunk(text, { carry = "", phases = DEFAULT_OUTBOUND_PHASES, threadId = null } = {}) {
  const parsed = parseThreadMirrorChunk(text, { carry, phases, threadId });
  return {
    trailingPartial: parsed.trailingPartial,
    messages: parsed.messages.filter((item) => item.role === "assistant"),
  };
}

function normalizeMirrorState(mirrorState, { threadId = null, rolloutPath = null } = {}) {
  return {
    initialized: mirrorState?.initialized === true,
    threadId: normalizeText(threadId || mirrorState?.threadId) || null,
    rolloutPath: normalizeText(rolloutPath || mirrorState?.rolloutPath) || null,
    byteOffset:
      Number.isFinite(mirrorState?.byteOffset) && Number(mirrorState.byteOffset) >= 0 ? Number(mirrorState.byteOffset) : 0,
    partialLine: typeof mirrorState?.partialLine === "string" ? mirrorState.partialLine : "",
    lastSignature: normalizeText(mirrorState?.lastSignature) || null,
    suppressions: Array.isArray(mirrorState?.suppressions) ? mirrorState.suppressions.map(String) : [],
    pendingMessages: Array.isArray(mirrorState?.pendingMessages) ? mirrorState.pendingMessages : [],
  };
}

function findLastSignatureIndex(messages, signature) {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    if (messages[index]?.signature === signature) {
      return index;
    }
  }
  return -1;
}

async function readFromOffset(filePath, offset, fileSize) {
  const length = Math.max(0, fileSize - offset);
  if (length === 0) {
    return "";
  }
  const handle = await fs.open(filePath, "r");
  try {
    const buffer = Buffer.alloc(length);
    const { bytesRead } = await handle.read(buffer, 0, length, offset);
    return buffer.subarray(0, bytesRead).toString("utf8");
  } finally {
    await handle.close();
  }
}

async function recoverMessagesFromTail({
  filePath,
  fileSize,
  lastSignature,
  phases,
  threadId,
  tailBytes = MAX_TAIL_RESYNC_BYTES,
}) {
  const normalizedSignature = normalizeText(lastSignature);
  if (!normalizedSignature || fileSize <= 0) {
    return null;
  }
  const offset = Math.max(0, fileSize - Math.max(0, tailBytes));
  const tailText = await readFromOffset(filePath, offset, fileSize);
  const { messages, trailingPartial } = parseThreadMirrorChunk(tailText, {
    phases,
    threadId,
  });
  const cursor = findLastSignatureIndex(messages, normalizedSignature);
  if (cursor < 0) {
    return null;
  }
  return {
    messages: messages.slice(cursor + 1),
    trailingPartial,
  };
}

export async function readThreadMirrorDelta({
  rolloutPath,
  mirrorState = null,
  threadId = null,
  phases = DEFAULT_OUTBOUND_PHASES,
}) {
  const normalizedPath = normalizeText(rolloutPath) || null;
  const nextBase = normalizeMirrorState(mirrorState, {
    threadId,
  });
  if (!normalizedPath) {
    return {
      messages: [],
      mirror: nextBase,
    };
  }

  let stats;
  try {
    stats = await fs.stat(normalizedPath);
  } catch (error) {
    if (error && typeof error === "object" && "code" in error && error.code === "ENOENT") {
      return {
        messages: [],
        mirror: {
          ...nextBase,
          rolloutPath: normalizedPath,
        },
      };
    }
    throw error;
  }

  const normalizedPhases = normalizePhases(phases);
  const fileSize = Number(stats.size) || 0;
  const pathChanged = Boolean(nextBase.rolloutPath && nextBase.rolloutPath !== normalizedPath);
  const truncated = nextBase.byteOffset > fileSize;
  const needsBootstrap = nextBase.initialized !== true;

  if (needsBootstrap) {
    return {
      messages: [],
      mirror: {
        ...nextBase,
        initialized: true,
        rolloutPath: normalizedPath,
        byteOffset: fileSize,
        partialLine: "",
      },
    };
  }

  if (pathChanged || truncated) {
    if (!nextBase.lastSignature || fileSize > MAX_FULL_RESYNC_BYTES) {
      return {
        messages: [],
        mirror: {
          ...nextBase,
          initialized: true,
          rolloutPath: normalizedPath,
          byteOffset: fileSize,
          partialLine: "",
        },
      };
    }

    const fullText = await fs.readFile(normalizedPath, "utf8");
    const { messages: snapshotMessages, trailingPartial } = parseThreadMirrorChunk(fullText, {
      phases: normalizedPhases,
      threadId,
    });
    let messages = [];
    const cursor = findLastSignatureIndex(snapshotMessages, nextBase.lastSignature);
    if (cursor >= 0) {
      messages = snapshotMessages.slice(cursor + 1);
    }
    return {
      messages,
      mirror: {
        ...nextBase,
        initialized: true,
        rolloutPath: normalizedPath,
        byteOffset: fileSize,
        partialLine: trailingPartial,
        lastSignature: nextBase.lastSignature,
      },
    };
  }

  if (fileSize === nextBase.byteOffset) {
    return {
      messages: [],
      mirror: {
        ...nextBase,
        rolloutPath: normalizedPath,
      },
    };
  }

  if (fileSize - nextBase.byteOffset > MAX_FULL_RESYNC_BYTES) {
    const recovered = await recoverMessagesFromTail({
      filePath: normalizedPath,
      fileSize,
      lastSignature: nextBase.lastSignature,
      phases: normalizedPhases,
      threadId,
    });
    if (recovered) {
      return {
        messages: recovered.messages,
        mirror: {
          ...nextBase,
          initialized: true,
          rolloutPath: normalizedPath,
          byteOffset: fileSize,
          partialLine: recovered.trailingPartial,
          lastSignature: nextBase.lastSignature,
        },
      };
    }
    return {
      messages: [],
      mirror: {
        ...nextBase,
        initialized: true,
        rolloutPath: normalizedPath,
        byteOffset: fileSize,
        partialLine: "",
      },
    };
  }

  const appendedText = await readFromOffset(normalizedPath, nextBase.byteOffset, fileSize);
  const { messages, trailingPartial } = parseThreadMirrorChunk(appendedText, {
    carry: nextBase.partialLine,
    phases: normalizedPhases,
    threadId,
  });
  return {
    messages,
    mirror: {
      ...nextBase,
      initialized: true,
      rolloutPath: normalizedPath,
      byteOffset: fileSize,
      partialLine: trailingPartial,
    },
  };
}
