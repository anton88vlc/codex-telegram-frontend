...
export function formatOutboundUserMirrorText(text, config = {}) {
  ...
  const displayName = normalizeText(config.codexUserDisplayName).replace(/\s+/g, " ") || "Codex Desktop";
  const label = /codex desktop user/i.test(displayName)
    ? "Codex Desktop"
    : /codex/i.test(displayName)
      ? displayName
      : `${displayName} via Codex`;
  return `**${label}**\n\n${normalized}`;
}
...
export function isImageGenerationMirrorMessage(message) {
  return message?.role === "assistant" && normalizeText(message?.phase) === "image_generation";
}
