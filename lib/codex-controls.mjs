...
import { appServerClientOptions } from "./app-server-transport.mjs";
...
export function getCurrentCodexTurnId(binding = {}) {
  return normalizeText(
    binding?.currentTurn?.appServerTurnId ||
      binding?.currentTurn?.turnId ||
      binding?.currentTurn?.id ||
      "",
  );
}

export function buildSteerTurnParams({ threadId, turnId, input }) {
  const normalizedThreadId = normalizeText(threadId);
  ...
  if (!normalizedTurnId) {
    throw new Error("missing active turn id");
  }
  return {
    threadId: normalizedThreadId,
    input: appServerTextInput(input),
    expectedTurnId: normalizedTurnId,
    responsesapiClientMetadata: null,
  };
}

export function buildInterruptTurnParams({ threadId, turnId }) {
  const normalizedThreadId = normalizeText(threadId);
  const normalizedTurnId = normalizeText(turnId);
  if (!normalizedThreadId) {
    throw new Error("missing thread id");
...
export async function steerCodexTurn(config, { threadId, turnId, input }) {
  const params = buildSteerTurnParams({ threadId, turnId, input });
  return withAppServerControl(config, (client) => client.request("turn/steer", params));
}

export async function interruptCodexTurn(config, { threadId, turnId }) {
  const params = buildInterruptTurnParams({ threadId, turnId });
  return withAppServerControl(config, (client) => client.request("turn/interrupt", params));
}
