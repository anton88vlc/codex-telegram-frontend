import { appServerTransportLabel } from "./app-server-transport.mjs";
...
export function buildSettingsReport({ config = {}, state = {}, bindingKey = null, binding = null } = {}) {
  ...
  const lines = [
    ...
    `ingress: poll ${formatDurationMs(Number(config.pollTimeoutSeconds) * 1000)}; typing ${formatOnOff(config.sendTyping)}; typing heartbeat ${formatOnOff(config.typingHeartbeatEnabled)} / ${formatDurationMs(config.typingHeartbeatIntervalMs)}; error cooldown ${formatDurationMs(config.typingHeartbeatErrorCooldownMs)}`,
    ...
    `transport: native; ingress ${config.nativeIngressTransport || "app-server"}; app-server \`${appServerTransportLabel(config)}\`; app-control \`${config.nativeDebugBaseUrl || "n/a"}\``,
    ...
    `private topics: experimental auto-create ${formatOnOff(config.privateTopicAutoCreateChats)}; cwd ${config.nativeChatStartCwd ? `\`${config.nativeChatStartCwd}\`` : "home default"}; start timeout ${formatDurationMs(config.nativeChatStartTimeoutMs)}`,
    `app-server stream: ${formatOnOff(config.appServerStreamEnabled)}; hot only ${formatOnOff(config.appServerStreamSubscribeHotOnly)}; connect ${formatDurationMs(config.appServerStreamConnectTimeoutMs)}; request ${formatDurationMs(config.appServerStreamRequestTimeoutMs)}; reconnect ${formatDurationMs(config.appServerStreamReconnectMs)}; resume attempts ${config.appServerStreamSubscribeMaxAttemptsPerPoll || 3}/poll; failure cooldown ${formatDurationMs(config.appServerStreamSubscribeFailureMaxDelayMs)}; duplicate errors ${formatOnOff(config.appServerStreamLogSubscribeErrors)}`,
    `mirror: ${formatOnOff(config.outboundSyncEnabled)}; phases ${formatList(config.outboundMirrorPhases)}; progress ${config.outboundProgressMode || "updates"}; poll ${formatDurationMs(config.outboundPollIntervalMs)}; max ${config.outboundMirrorMaxBindingsPerPoll || "all"}/poll`,
    `working set: hot ${formatDurationMs(config.bindingHotMaxAgeMs)}; warm ${formatDurationMs(config.bindingWarmMaxAgeMs)}`,
    ...
    `status bar: ${formatOnOff(config.statusBarEnabled)}; pin ${formatOnOff(config.statusBarPin)}; tail ${formatBytes(config.statusBarTailBytes)}; Codex config poll ${formatDurationMs(config.statusBarCodexConfigPollIntervalMs)}`,
    `sync: default working set ${config.syncDefaultLimit || 3}; project index \`${config.projectIndexPath || "state/bootstrap-result.json"}\``,
    `topic auto-sync: ${formatOnOff(config.topicAutoSyncEnabled)}; limit ${config.topicAutoSyncLimit || config.syncDefaultLimit || 3}; poll ${formatDurationMs(config.topicAutoSyncPollIntervalMs)}; max age ${formatDurationMs(config.topicAutoSyncMaxThreadAgeMs)}; max actions ${config.topicAutoSyncMaxActionsPerTick || 8}`,
    `paths: state \`${config.statePath || "state/state.json"}\`; event log \`${config.eventLogPath || "logs/bridge.events.ndjson"}\` max ${formatBytes(config.eventLogMaxBytes)}; stderr \`${config.bridgeLogPath || "logs/bridge.stderr.log"}\`; threads DB \`${config.threadsDbPath || "~/.codex/state_5.sqlite"}\``,
