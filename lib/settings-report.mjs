...
export function buildSettingsReport({ config = {}, state = {}, bindingKey = null, binding = null } = {}) {
  ...
  const lines = [
    ...
    `private topics: auto-create Chats ${formatOnOff(config.privateTopicAutoCreateChats)}; cwd ${config.nativeChatStartCwd ? `\`${config.nativeChatStartCwd}\`` : "home default"}; start timeout ${formatDurationMs(config.nativeChatStartTimeoutMs)}`,
