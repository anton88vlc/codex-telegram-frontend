...
export function buildSettingsReport({ config = {}, state = {}, bindingKey = null, binding = null } = {}) {
  ...
  const lines = [
    ...
    `transport: native; ingress ${config.nativeIngressTransport || "app-server"}; app-server \`${config.appServerUrl || "n/a"}\`; app-control \`${config.nativeDebugBaseUrl || "n/a"}\``,
    ...
    `app-server stream: ${formatOnOff(config.appServerStreamEnabled)}; hot only ${formatOnOff(config.appServerStreamSubscribeHotOnly)}; connect ${formatDurationMs(config.appServerStreamConnectTimeoutMs)}; reconnect ${formatDurationMs(config.appServerStreamReconnectMs)}; resume attempts ${config.appServerStreamSubscribeMaxAttemptsPerPoll || 3}/poll`,
