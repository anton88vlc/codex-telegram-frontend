...
export function buildSettingsReport({ config = {}, state = {}, bindingKey = null, binding = null } = {}) {
  ...
  const lines = [
    ...
    `transport: native; app-control \`${config.nativeDebugBaseUrl || "n/a"}\`; fallback \`${config.appServerUrl || "n/a"}\``,
    `timeouts: send ${formatDurationMs(config.nativeTimeoutMs)}; native poll ${formatDurationMs(config.nativePollIntervalMs)}`,
