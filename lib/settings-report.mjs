...
export function buildSettingsReport({ config = {}, state = {}, bindingKey = null, binding = null } = {}) {
  ...
  const lines = [
    ...
    `app-server stream: ${formatOnOff(config.appServerStreamEnabled)}; connect ${formatDurationMs(config.appServerStreamConnectTimeoutMs)}; reconnect ${formatDurationMs(config.appServerStreamReconnectMs)}; resume attempts ${config.appServerStreamSubscribeMaxAttemptsPerPoll || 3}/poll`,
