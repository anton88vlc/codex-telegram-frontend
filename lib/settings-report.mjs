...
export function buildSettingsReport({ config = {}, state = {}, bindingKey = null, binding = null } = {}) {
  ...
  const lines = [
    ...
    `timeouts: send ${formatDurationMs(config.nativeTimeoutMs)}; native poll ${formatDurationMs(config.nativePollIntervalMs)}; app-control cooldown ${formatDurationMs(config.appControlCooldownMs)}`,
    `mirror: ${formatOnOff(config.outboundSyncEnabled)}; phases ${formatList(config.outboundMirrorPhases)}; progress ${config.outboundProgressMode || "updates"}; complete ${config.outboundProgressCompleteMode || "delete"}; poll ${formatDurationMs(config.outboundPollIntervalMs)}`,
    ...
    `paths: state \`${config.statePath || "state/state.json"}\`; event log \`${config.bridgeLogPath || "logs/bridge.stderr.log"}\`; threads DB \`${config.threadsDbPath || "~/.codex/state_5.sqlite"}\``,
