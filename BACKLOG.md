...
37. Private bot topic auto-create: the first user message in an unbound bot-private topic creates a Desktop-visible Codex Chat from the user's home cwd and starts that same first turn in one app-server `thread/start + turn/start` move. No dead `New Thread`, no second resume attempt into a thread Codex has not fully warmed up yet.
