...
4. Managed bots and `t.me/newbot/...` links. This is the big onboarding simplifier: fewer BotFather gymnastics, cleaner token handoff, maybe a future manager-bot flow. Keep it local/user-owned; do not turn the project into a weird SaaS control plane. Tracked in Linear as `VLC-699`.
...
6. ~~Native Telegram checklists for Codex Todo spike.~~ Still parked: `sendChecklist` and `editMessageChecklist` require a connected business account (`business_connection_id`), so they are not a sane default for normal bot/project groups. Keep text Todo blocks.
...
11. ~~Agent-led onboarding automation hardening base: `onboard:prepare` creates local config/admin env files, can set up the admin Python venv, guides credential wiring and uses phone/code login as the default Telegram session path.~~
12. ~~Onboarding recovery hardening for bad-but-present Telegram credentials/session errors, not only missing files.~~
13. ~~Real clean-install audit hardening: agents must not collect secrets in Codex chat, `prepare` explains Telegram API credentials before asking, verifies stale sessions before skipping login, phone login is the default path and quickstart backfill skips empty threads instead of killing the whole run.~~
...
5. Managed-bot onboarding spike (`VLC-699`): Bot API 9.6 added managed bots and `t.me/newbot/...` links. Investigate whether a manager bot can make first install less BotFather-heavy without adding a creepy SaaS control plane.
6. ~~Native Telegram Checklist spike for Codex Todo blocks.~~ Validated against the current Bot API: checklist send/edit is business-account-only, so text Todo remains the correct default.
...
40. Bridge refactor base: outbound mirror text, progress message send/edit, status bar runner, app-server stream runner and typing heartbeat runner are now outside `bridge.mjs` with focused tests. This started the diet; later slices made the entrypoint properly boring.
...
57. Draft streaming for private Codex Chats: runtime support exists, but it is off by default because Telegram currently rejects empty clear attempts and can leave stale "Working..." draft bubbles. Project groups deliberately stay on progress bubbles.
58. Generated Codex images mirror into Telegram as real photos: rollout `image_generation_call` items and live app-server media events resolve to local Codex generated-image files and send through Bot API `sendPhoto`; duplicate suppression keeps the two mirror paths from posting the same image twice, and empty app-server status ticks no longer create random night-time progress bubbles.
