...
58. Generated Codex images mirror into Telegram as real photos: rollout `image_generation_call` items resolve to local Codex generated-image files and send through Bot API `sendPhoto`; empty app-server status ticks no longer create random night-time progress bubbles.
