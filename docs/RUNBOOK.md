...
For project groups, bootstrap generates a compact themed avatar from the group title when the group has no custom photo yet. It will not overwrite a human-picked photo unless you pass `--refresh-group-avatars`; use `--skip-group-avatars` if you want raw Telegram initials.
It merges groups into `state/bootstrap-result.json`; pass `--replace-result` only for an intentional clean rebuild.

To repaint existing project groups from the current bootstrap index:

```bash
npm run group:avatars
```

That deliberately overwrites group photos. For a no-write preview:

```bash
admin/.venv/bin/python admin/telegram_user_admin.py refresh-group-avatars --dry-run
```
